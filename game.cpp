...
// 2v2: teammate trails are passable
...
static void spawn_players(GameMode mode, Slot slots[4]) {
    ...
    struct { int x, y; Dir d; } sp2[] = {
        {GW/4, GH/2, D_RIGHT}, {3*GW/4, GH/2, D_LEFT},
    };
    struct { int x, y; Dir d; } sp4[] = {
        {GW/4, GH/4, D_RIGHT},     {3*GW/4, GH/4, D_LEFT},
        {GW/4, 3*GH/4, D_RIGHT},   {3*GW/4, 3*GH/4, D_LEFT},
    };
    for (int i=0; i<num_players; i++) {
        Player& p = players[i];
        p.slot = &slots[i]; p.cell = (Cell)(C_P1+i); p.index = i; p.alive = true;
        if (num_players==2) { p.x=sp2[i].x; p.y=sp2[i].y; p.dir=sp2[i].d; }
        else                { p.x=sp4[i].x; p.y=sp4[i].y; p.dir=sp4[i].d; }
...
static void draw_hud(GameMode mode) {
    ...
    for (int i=0; i<num_players; i++) {
        char buf[32];
        snprintf(buf, 32, "%s%d%s ", players[i].slot->human?"P":"CPU",
                 i+1, players[i].alive?"●":"✕");
        hud += buf;
...
static void ai_think(Player& p, GameMode mode) {
    ...
    for (int d=0; d<4; d++) {
        ...
        // hard: also check perpendicular room
...
static void move_player(Player& p, GameMode mode) {
    ...
    if (blocked_for(nx, ny, p.slot->team, mode, grid)) { p.alive = false; return; }
...
static int handle_input() {
    ...
    for (int i=0; i<num_players; i++) {
        ...
        Dir nd = D_NONE;
        if (ch==ks.up) nd=D_UP; if (ch==ks.down) nd=D_DOWN;
        if (ch==ks.left) nd=D_LEFT; if (ch==ks.right) nd=D_RIGHT;
